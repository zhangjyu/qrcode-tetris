...
bool TetrisLayer::checkCoverByPos(int row, int col)
{
    ...
    for (int i = 0; i < rowColBlocks.size(); i++)
    {
        ...
        if (tempRow < lowRow || tempCol < 0 || tempCol > _nColNums-1)
            return true;
...
void TetrisLayer::upline(int line) {
	log("%d",_nRowNums);
	for (int moveRow = _nRowNums; moveRow >= 0;--moveRow) {
		...
		for (int moveCol = 0; moveCol < _nColNums; ++moveCol) {
			...
			auto block = dynamic_cast<Block*>(getChildByTag(id));
			if (block) {
				auto newId = getRowColumnID(moveRow + line, moveCol);
				block->setTag(newId);
				block->setRow(moveRow + line);
				auto pos = getPositionByRowCol(moveRow + line, moveCol);
				block->setPosition(pos);
			}/*
			for (int col = 0; col < _nColNums; ++col) {
